...
def partition(lst, n):
  """Divide list into n nearly equal chunks"""
  k, m = divmod(len(lst), n)
  return [lst[i*k + min(i,m): (i+1)*k + min(i +1, m)] for i in range(n)]

# Find the total *.jpg files in a given folder
def get_image_count(input_pdf_dir):
  count = 0
  for _, _, files in os.walk(input_pdf_dir):
    for file in files:
...
#   print(get_image_count("/workspace/documents"))