...
class Infer:
	def __init__(self):
		...
		self.model = TLDr_Trainer.load_from_checkpoint(f"/home/layout/layout-parser/models/linetrckp/best_scr.ckpt")
		self.model.freeze()
		self.bin_model = TLDr_BinDecoder.load_from_checkpoint(f"/home/layout/layout-parser/models/linetrckp/best_bin.ckpt", encoder=self.model.encoder)
		...
		with open(f"/home/layout/layout-parser/models/linetrckp/colors_new.pkl", "rb") as f:
	...
	def reproject(self):
		...
		# self.global_weight_canvas[self.global_weight_canvas < 1.0] = 0.0
	...
	def sample_patches(self, patch_size, stride_x, stride_y):
		...
		for tile in img_tiles:
			...
			tile["tensor"] = (torch.from_numpy(tile["img"]) / 255.0 - 0.5).permute(2, 0, 1)
	...
	def process_image(self, img_path):
		...
		# saving the heat map ------------------ ######## DEBUG - commented saving heatmaps
		# heatmap = np.repeat(self.global_heat_map[:, :, None], 3, axis=-1)

		# print(f'\n\n\n\n Saving Outputs - {os.getcwd()} \n\n\n\n') ### DEBUG
		# if self.name is not None:
		# 	cv2.imwrite(f"outputs/heatmaps_{self.name}/{img_path.split('/')[-1]}", heatmap)
		# else:
		# 	cv2.imwrite(f"outputs/heatmap_{img_path.split('/')[-1]}", heatmap)
		...
		# returning the final seams
		return upper_seams_xy, lower_seams_xy, self.img
	...
	# 	print(f'\n\n\n\n Saving Pickle file - {os.getcwd()} - {DATASET}_RESULT.pkl \n\n\n\n') ### DEBUG
...
# pi.process_image("btv1b8455939z.jpg")
